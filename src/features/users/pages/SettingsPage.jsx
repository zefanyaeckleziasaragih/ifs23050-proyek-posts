...
import {
  showSuccessDialog,
  showErrorDialog,
} from "../../../helpers/toolsHelper"; // <--- IMPORT DIALOG

function SettingsPage() {
  const [loading, setLoading] = useState(true);
  const [fadeIn, setFadeIn] = useState(false);
  // Hapus state isDarkMode dan language

  // Definisikan kembali konstanta CSS dari ProfilePage
  const PRIMARY_COLOR = "#667eea"; // Dari ProfilePage (Biru)
  const ACCENT_COLOR = "#f58529"; // Dari Gradient tombol di ProfilePage (Oranye)
  ...
  // Hapus fungsi toggleDarkMode

  const handleSaveSettings = () => {
    // Simulasi proses penyimpanan (tombol ini sekarang bisa digunakan untuk simulasi simpan umum)
    try {
  ...
  // FUNGSI BARU: Kirim Laporan (Simulasi)
  const handleSendReport = () => {
    // Di sini Anda bisa memicu modal/form untuk diisi pengguna, tapi untuk demo, kita pakai dialog.
    showSuccessDialog(
      "Laporan Anda berhasil dikirim. Kami akan segera memeriksanya!"
    );
  };

  // FUNGSI BARU: Buka Email
  ...
  return (
    <>
      ...
      <div
        style={{
          background: lightModeStyles.mainBg,
          minHeight: "100%",
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {/* --- PENGATURAN UMUM --- (Disederhanakan) */}
              <div
